      ...
      case 'scroll':
        {
          ...
          if (state.isTouching && !state.isDragging) {
            ...
            if (props.onScrollDragStart) {
              dispatchEvent(event, context, state, 'scrolldragstart', props.onScrollDragStart, UserBlockingEvent);
            }
          }
          if (props.onScroll) {
      ...
      case 'touchend':
        {
          if (state.isTouching) {
            if (state.isDragging && props.onScrollDragEnd) {
              dispatchEvent(event, context, state, 'scrolldragend', props.onScrollDragEnd, UserBlockingEvent);
            }
            state.isTouching = false;
            state.isDragging = false;
            state.scrollTarget = null;
