...
function handleRootPointerEvent(event, context, state, callback) {
  ...
  if (focusTarget !== null && context.isTargetWithinNode(event.target, focusTarget) && (type === 'mousedown' || type === 'touchstart' || type === 'pointerdown')) {
    callback(false);
  }
}
...
function dispatchBlurEvents(context, props, state) {
  ...
  if (props.onFocusChange) {
    ...
    var _syntheticEvent2 = createFocusEvent(context, 'focuschange', target, pointerType);
...
function dispatchFocusWithinChangeEvent(context, props, state, value) {
  ...
  if (props.onFocusWithinChange) {
    ...
    var syntheticEvent = createFocusEvent(context, 'focuswithinchange', target, pointerType);
    context.dispatchEvent(syntheticEvent, listener, DiscreteEvent);
  }
  if (state.isFocusVisible) {
...
function dispatchFocusWithinVisibleChangeEvent(context, props, state, value) {
  ...
  if (props.onFocusWithinVisibleChange) {
    ...
    var syntheticEvent = createFocusEvent(context, 'focuswithinvisiblechange', target, pointerType);
    context.dispatchEvent(syntheticEvent, listener, DiscreteEvent);
  }
}
...
var FocusWithinResponder = {
  ...
  onEvent: function (event, context, props, state) {
    ...
    switch (type) {
      ...
      case 'blur':
        {
          if (state.isFocused && !context.isTargetWithinEventResponderScope(relatedTarget)) {
            dispatchFocusWithinChangeEvent(context, props, state, false);
            state.isFocused = false;
          }
          break;
