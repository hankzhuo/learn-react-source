...
function FiberNode(tag, pendingProps, key, mode) {
  ...
  this.pendingProps = pendingProps;  // 待更新的 props
  this.memoizedProps = null; // 更新完后的 props
  this.updateQueue = null;  // 更新队列
  this.memoizedState = null; // 渲染完的 state
...
function FiberRootNode(containerInfo, tag, hydrate) { // fiber root 节点
  this.tag = tag;
  this.current = null;
...
function createFiberRoot(containerInfo, tag, hydrate) { // 创建 fiber root
...
function createContainer(containerInfo, tag, hydrate) { // 创建 fiberRoot
...
function ReactSyncRoot(container, tag, hydrate) {
  ...
  var root = createContainer(container, tag, hydrate);  // 创建了 fiber root
...
function getReactRootElementInContainer(container) {
  ...
  if (container.nodeType === DOCUMENT_NODE) { // DOCUMENT_NODE === doucment.nodeType === 9
...
function shouldHydrateDueToLegacyHeuristic(container) { // container 为 DOM 节点
...
function legacyCreateRootFromDOMContainer(container, forceHydrate) {
  var shouldHydrate = forceHydrate || shouldHydrateDueToLegacyHeuristic(container); // 根据节点类型是否混合更新
...
function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
  ...
  if (!root) {
    // Initial mount
    root = container._reactRootContainer = legacyCreateRootFromDOMContainer(container, forceHydrate); // 创建 Root
