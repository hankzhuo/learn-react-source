...
var eventListeners = new PossiblyWeakMap();
...
var eventResponderContext = {
  ...
  removeRootEventTypes: function(rootEventTypes) {
    ...
    for (var i = 0; i < rootEventTypes.length; i++) {
      ...
      var rootEventComponents = rootEventTypesToEventComponentInstances.get(
        rootEventType
      );
...
function processEventQueue() {
  var _ref = currentEventQueue,
    events = _ref.events,
    eventPriority = _ref.eventPriority;
...
function triggerOwnershipListeners() {
  ...
  try {
    for (var i = 0; i < listeningInstances.length; i++) {
      ...
      var _isHook2 = _instance2.isHook,
...
function handleTargetEventResponderInstance(
  ...
  if (targetEventTypes !== undefined) {
    if (responderEventTypesContainType(targetEventTypes, topLevelType)) {
      ...
      var _isHook3 = eventComponentInstance.isHook,
        props = eventComponentInstance.props,
        state = eventComponentInstance.state;

      var eventListener = responder.onEvent;
...
function traverseAndHandleEventResponderInstances(
  ...
  var hookComponentResponderValidation = new Set();
  ...
  if (rootEventInstances !== undefined) {
    ...
    for (var _i = 0; _i < rootEventComponentInstances.length; _i++) {
      ...
      var _isHook4 = rootEventComponentInstance.isHook,
        props = rootEventComponentInstance.props,
        responder = rootEventComponentInstance.responder,
...
function addRootEventTypesForComponentInstance(
  eventComponentInstance,
  rootEventTypes
) {
  for (var i = 0; i < rootEventTypes.length; i++) {
...
function updateEventComponentInstance(eventComponent, props) {
  ...
  if (currentEventComponentInstanceIndex === events.length) {
    ...
    var eventComponentInstance = createEventComponentInstance(
